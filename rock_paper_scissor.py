...
def result(human_choice,comp_choice):
    ...
    #Text
    text_area=tk.Text(master=window,height=5,width=30)
    text_area.grid(column=15,row=4)
    answer="Your Choice: {uc} \nBot Choice : {cc} \n-----------------------------\nYour Score : {u} \nBot Score : {c}  \n\n  ".format(uc=USER_CHOICE,cc=COMP_CHOICE,u=USER_SCORE,c=COMP_SCORE, font=('arial',24,'bold'))
    if(not(user==comp)):
